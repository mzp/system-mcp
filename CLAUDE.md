...
make install                    # release + 署名 + ~/.local/bin/systemmcp へ原子的コピー（INSTALL_DIR で上書き可）
make sign SIGN_IDENTITY="Developer ID Application: ..."   # Developer ID 署名
```

`make install` は署名済みバイナリを安定パス `~/.local/bin/systemmcp`（`$PATH` 上。hermes / Claude Desktop /
シェルから使う実体）へ置く。`.build/release` は再ビルドで上書きされるため直接運用しない。
**コピーは temp + `mv` の原子的差し替えで行う**：署名済み Mach-O を同一 inode に `cp` で上書きすると、
カーネルがキャッシュ済みページハッシュとの不一致を検出し、次回 exec を `Killed: 9`（exit 137）で殺す。
新 inode へ rename すればこれを回避でき、バイト列が同一なので cdhash も不変＝既存の TCC 許可が維持される。
...
`skills/apple-reminder/SKILL.md`・`skills/apple-calendar/SKILL.md` は、hermes agent が
**MCP ではなく CLI 版**（`~/.local/bin/systemmcp reminder/calendar ...`）を terminal ツールで
直接叩くためのスキル。CLI のフラグ表面と密に対応するので、サブコマンドや `--option` を増減したら
該当 SKILL.md も更新する。配置先（hermes への symlink/コピー）は別リポ `hermes-setup` 側の管轄で、
本リポはスキル定義の正本だけを持つ。

- terminal は `terminal.backend: local` 必須（docker サンドボックスからはホストのバイナリが見えない）。
- TCC: gateway/Discord 経由なら systemmcp が自前の責任プロセスになり `fullAccess`。SKILL.md に
  `disclaimer` は書かない（あれは Claude Code からテストする時だけ必要な再現用ラッパー）。

E2E 回帰テストは `skills/eval/run.sh`（実プロンプトを `hermes chat -m <model>` に流し、agent が出した
`systemmcp` コマンドと実効果を検証。書き込みケースは追加分を差分検出して消す自己クリーニング型。既定モデルは
weak の床として `minimax/minimax-m3`）。Claude Code からは `.claude/skills/skill-eval` 経由で起動できる。
SKILL.md のフラグや決定表を変えたらこれを回す。`disclaimer` ラップ前提なのでこの Mac でのみ動く（CI 不可）。
